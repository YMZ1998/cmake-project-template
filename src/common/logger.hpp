...
#define STM_TRACE()                                          \
  logger::log_stream({__FILENAME__, __LINE__, __FUNCTION__}, \
                     spdlog::level::trace, "")
#define STM_DEBUG()                                          \
  logger::log_stream({__FILENAME__, __LINE__, __FUNCTION__}, \
                     spdlog::level::debug, "")
...
#define PRINT_TRACE(fmt, ...)                                        \
  wlog::logger::get().printf({__FILENAME__, __LINE__, __FUNCTION__}, \
                             spdlog::level::trace, fmt, ##__VA_ARGS__);
#define STREAM_TRACE()                                             \
  wlog::logger::log_stream({__FILENAME__, __LINE__, __FUNCTION__}, \
                           spdlog::level::trace, "")
#else
#define LOG_TRACE(fmt, ...)
#define PRINT_TRACE(fmt, ...)
...
#define LOG_ERROR(fmt, ...)                                                 \
  spdlog::log({__FILENAME__, __LINE__, __FUNCTION__}, spdlog::level::error, \
              fmt, ##__VA_ARGS__);
#define PRINT_ERROR(fmt, ...)                                        \
  wlog::logger::get().printf({__FILENAME__, __LINE__, __FUNCTION__}, \
                             spdlog::level::error, fmt, ##__VA_ARGS__);
#define STREAM_ERROR()                                             \
  wlog::logger::log_stream({__FILENAME__, __LINE__, __FUNCTION__}, \
                           spdlog::level::error, "")
#else
...
#define LOG_FATAL(fmt, ...)                                                 \
  spdlog::log({__FILENAME__, __LINE__, __FUNCTION__}, spdlog::level::fatal, \
              fmt, ##__VA_ARGS__);
#define PRINT_FATAL(fmt, ...)                                        \
  wlog::logger::get().printf({__FILENAME__, __LINE__, __FUNCTION__}, \
                             spdlog::level::fatal, fmt, ##__VA_ARGS__);
#define STREAM_FATAL()                                             \
  wlog::logger::log_stream({__FILENAME__, __LINE__, __FUNCTION__}, \
                           spdlog::level::fatal, "")
#else
